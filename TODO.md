...
- ✅ **DECIDED 2026-09-18: Romanian stays in the app** (it is the only complete locale besides EN, costs nothing to keep, and RO is the cheapest test market + a large euro-paying diaspora). It just drops from the international marketing narrative: order the language picker by market size, replace the Romanian seeded leaderboard names, fix the Romanian month abbreviations in `zodiac.ts` date ranges. Full reasoning + the launch audit in `AUDIT.md`.
...
16. ✅ **User accounts / login** — DONE 2026-09-12 (code) — Supabase auth, **optional** (app still works offline). Email/password + password reset fully wired, Google/Apple via OAuth (PKCE + expo-web-browser). `services/auth.ts` + `authValidate.ts` (5 tests) + `AccountModal` + an Account row in Profile. Leaderboard stays on Cloudflare D1. **ACTION NEEDED FROM GEORGE:** create a free Supabase project, put `EXPO_PUBLIC_SUPABASE_URL` + `EXPO_PUBLIC_SUPABASE_ANON_KEY` in `.env` (see `AUTH_SETUP.md`). Email/password works right after that; Google needs provider config + a prebuild for the `clarmind://` scheme; Apple needs the Apple dev account.
19. ✅ **Cloud sync (accounts save your data)** — DONE 2026-09-17 — signing in backs up the whole app state (birth details, streaks, sessions, moods, prefs, leaderboard device id) to Supabase as one JSON blob (`user_data` table + RLS in `supabase/user_data.sql`) and restores it on a new device. Newest-wins (`syncLogic.decideSync`, tested); syncs on app start when signed in + on background; "Back up now" in Account. Reuses `backup.ts`, so restore also carries the leaderboard identity. Privacy page + AUTH_SETUP updated. **ACTION: run `supabase/user_data.sql` in the Supabase SQL editor.**
16b. ⏳ **User accounts / login** (superseded by #16) — sign in with Google, Apple, or email + password; forgot-password flow, account management, cross-device sync. Needs an auth backend (e.g. Supabase Auth, Firebase Auth, or Clerk) and moving from local-only storage to a synced user record. Big piece, defer until after launch. Ties into the friends/social backlog.
...
| 2026-09-18 | Launch audit | Full pre-launch audit written to **`AUDIT.md`**: localization gap (it/fr/es/de/pt each miss the same 263 of 527 keys, including the language-limit alerts; hardcoded English in notifications, streak badge, zodiac date ranges, astro/numerology tables), 12 P0 launch blockers (testing-premium toggle visible in free build, open AI proxy, `Alert.alert` no-op on web, UTC day boundary, no keep-awake in sessions, English-only notifications, Nominatim autocomplete policy, store compliance, birth time treated as UT), ~60 more findings, feature ideas, monetization/GTM plan. Appendices: drop-in locale parity test, verified drafts of all 263 keys x5 languages, new keys for the hardcoded text. Decision: keep Romanian. Nothing implemented yet. |
| 2026-09-23 | Audit quick wins | Worked the easy/contained items from AUDIT.md. **DONE:** P0 #1 i18n — merged Appendix B so es/it/fr/de/pt reach full parity (527 keys), swept the 12 em dashes/file, flipped `DeepPartial`→`TranslationShape` (missing key = compile error), added the parity test + `i18n:check` (this fixes George's Spanish-shows-English report). P0 #2 hide Premium-testing toggle behind `__DEV__` + blocklist `premium_override`/`device_id` on backup import. P0 #4 `showDialog` helper so web alerts/confirms work. Item 18 version alignment (package 1.7.1, footer from expo-constants, versionCode 8) + PWA renamed Stillnova. Item 23 language-aware insight cache. Item 27 notif handler guarded off web. Item 9 iOS `ITSAppUsesNonExemptEncryption:false` + `CFBundleLocalizations`. 314 tests pass, typecheck clean, 7 commits pushed. |
| 2026-09-23 | Deeper P0s #5 + #10 | **P0 #5 UTC day boundary:** added pure `localDateKey()` to `streakLogic.ts` and routed every "today" key through it (streak/last-open, AI quota, session date, XP/challenges/guide-read, course start+unlock, daily-content + numerology cache, Sky today/yesterday, constellation, reflection, heatmap); kept `weeklyRecapLogic` internally consistent (local today + local mood-day); `getMondayKey` stays UTC by design. **P0 #10 birth time as UT:** added `tz-lookup` (offline), convert local birth time -> UT via the birth place's IANA zone (DST-aware, date rollover handled) in `birthChart.ts` `computeNatalChart`/`moonSign`; device-zone fallback flagged via `NatalChart.exactTime`/`zone`; Intl failures degrade gracefully. 322 tests pass. Requested order: do #5 and #10 first, leave #9 last. **P0 #3 open AI proxy DONE 2026-09-23:** `proxy/worker.js` now enforces `X-App-Key` (only when the `APP_KEY` secret is set), CORS limited to the web origin, server-side `maxOutputTokens` clamp, and a per-device daily cap in Workers KV (skipped if `RATE_KV` unbound); client sends `X-App-Key`/`X-Device-Id` via `proxyHeaders()` in `ai.ts`, used by `tts.ts` too. **Activation needs George:** `wrangler deploy` + `wrangler secret put APP_KEY` + create `RATE_KV`, then rebuild the app with `EXPO_PUBLIC_AI_APP_KEY`. Secondary from #10 not done: NumerologyScreen shows self-selected `zodiacSign` as Sun (offer to correct from computed Sun); UI copy for inexact birth zone (needs 7-lang i18n). |
| 2026-09-23 | Deeper P0s #6 + #7 + #8 | **P0 #6 keep-awake:** `useKeepAwake` during an active breathing session and while a guided meditation is spoken; breathing countdown now driven by a wall-clock `endsAt` deadline + AppState 'active' reconcile so a backgrounded timer still ends correctly; audio `staysActiveInBackground: true` + iOS `UIBackgroundModes:['audio']`; declared `expo-keep-awake`. **P0 #7 notifications:** added localized `notifications.*` to all 7 locales, body via `translate()` + localized `rankName`, Android channel named + `channelId` passed, reschedule on every app open so it rotates and follows language. **P0 #8 geocoding:** moved birth-place autocomplete AND one-shot geocode off Nominatim (autocomplete violates its policy) to Photon (keyless, built for type-ahead); filter to populated places, honor country client-side, pass app language. 322 tests pass. **STILL OPEN P0: only #9** (account deletion + Sign in with Apple + paywall Terms/Privacy legal links) — LEFT UNDONE on purpose: needs George's Apple Developer + Supabase Edge Function setup, Claude can't do it. All other P0s (#1-#8, #10) done. |
...
| 2026-09-24 | P1 #16/#19/#28 + CI | **#16 sync merge:** `syncOnLogin` now MERGES local+cloud (union append-only collections, max counters, scalars by newer side) instead of destructive newest-wins, so a two-device user never loses offline activity; pure `mergeBlobs` unit-tested. Also fixed the regression from #2: cloud restore is a "trusted" path (`restoreState`) that keeps `clarmind_device_id` for identity continuity while pasted backups still drop it. **#19:** untracked the 15 committed APKs + gitignore `*.apk/*.aab` (history rewrite left as a coordinated follow-up). **#28:** KeyboardAvoidingView iOS-only (Android uses pan). **#30:** added CI workflow (typecheck + tests on push/PR). 364 tests. Built Stillnova 1.7.2 (versionCode 9) with everything. |

---

**Last updated:** 2026-09-24 (Done: P0 #1-#8+#10; P1 #11-#16, #18, #19, #21, #28; #30 CI; paywall annual/monthly+trial+legal (the paywall sub-item of #9); 364 tests. Built Stillnova 1.7.2 vc9. REMAINING #9 (George): in-app account deletion + Sign in with Apple. Open non-#9 items left as product decisions / larger efforts: #17 leaderboard consent + exclude chat from sync, #20 accessibility pass, #22 quota covers all AI calls, #24 guide-read XP without scroll, #25 streak = session-day (product decision), #26 per-language daily cache, #27 web linking/module-scope Dimensions, #29 crash reporting (needs Sentry DSN), #31/#32 onboarding+notif UX, #33 remote rank, #34 expo-av->expo-audio (post-launch), #35 dead code, #36 minor correctness. George also owns: create store products + RevenueCat offering; wrangler deploy the hardened AI proxy AND leaderboard-worker.)
